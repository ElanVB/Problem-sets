public class Erlang
{
  ...
  public static double nonrecErlang(int n, double p)
  {
    ...
    for(int i = 0; i <= n; i++)
    {
      sum += Math.pow(p, i)/(double)factorial(i);
    }
    result = (Math.pow(p, n)/(double)factorial(n))/sum;
  ...
  private static int factorial(int n)
{
  ...
  public static void main(String[] args)
  {
    ...
    System.out.println("Non-Recursive: " + nonrecErlang(Integer.parseInt(args[0]), Double.parseDouble(args[1])));
  }
}